...
interface MapViewProps {
  alerts: (WaterAlert & { isReport?: boolean })[];
  zones?: any[];
...
const SEVERITY_COLORS = {
  ...
  medium: '#ff9100',
  low: '#00e5ff',
  report: '#3d85ff', // Blue for reports
} as const;
...
function createPopupHTML(alert: WaterAlert & { isReport?: boolean }): string {
  const severityKey = alert.isReport ? 'report' : alert.severity;
  const severityColor = SEVERITY_COLORS[severityKey];
  const severityLabel = alert.isReport ? 'Report' :
...
export default function MapView({
  ...
  /* ── Update GeoJSON when city changes or zones change ───────────────────────── */
  useEffect(() => {
    if (!mapRef.current || !mapLoaded) return;
  ...
  const createMarkerElement = useCallback(
    (alert: WaterAlert & { isReport?: boolean }, isSelected: boolean): HTMLDivElement => {
      const el = document.createElement('div');
      el.className = 'map-marker';
      const severityKey = alert.isReport ? 'report' : alert.severity;
      const color      = SEVERITY_COLORS[severityKey];
      const outerColor = SEVERITY_OUTER[severityKey];
