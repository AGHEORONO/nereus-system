...
/** Generate mock satellite GeoJSON features (lake/river) for a new city's bounding box */
export function generateMockSatelliteDataForCity(
  bbox: { south: number; north: number; west: number; east: number },
  cityName: string
  ...
  // Fake River (cutting across)
  const riverCoords = [
    [bbox.west, centerLat + 0.01],
    [centerLng, centerLat],
    [bbox.east, centerLat - 0.01],
    [bbox.east, centerLat - 0.015],
    [centerLng, centerLat - 0.005],
    [bbox.west, centerLat + 0.005],
    [bbox.west, centerLat + 0.01],
  ];

  // Fake Lake
  const lakeLat = centerLat + (Math.random() > 0.5 ? 0.02 : -0.02);
  const lakeLng = centerLng + (Math.random() > 0.5 ? 0.02 : -0.02);
  const r = 0.005; // radius roughly
  const lakeCoords = [
    [lakeLng - r, lakeLat - r],
    [lakeLng + r, lakeLat - r * 0.5],
    [lakeLng + r * 1.5, lakeLat + r],
    [lakeLng, lakeLat + r * 1.2],
    [lakeLng - r, lakeLat - r],
  ];
  ...
  const features: GeoJSON.Feature[] = [
    {
      ...
      properties: {
        name: `${cityName} River`,
        type: 'river',
        ndwi: 0.65 + Math.random() * 0.2, // clear water
        ndvi: 0.1,
        ndvi_water: 0.15 + Math.random() * 0.2, // slight silt
        ndsi: 0.01,
        swir: 'water',
        area_km2: '1.2',
        passDate: new Date().toISOString()
      },
      geometry: { type: 'Polygon', coordinates: [riverCoords] }
    },
    {
      type: 'Feature',
      id: `mock-sat-2`,
      properties: {
        name: `${cityName} Lake`,
        type: 'lake',
        ndwi: 0.8 + Math.random() * 0.1, // very clear
        ndvi: 0.05,
        ndvi_water: 0.6 + Math.random() * 0.2, // algae bloom
        ndsi: 0.01,
        swir: 'vegetation',
        area_km2: '0.8',
        passDate: new Date().toISOString()
      },
      geometry: { type: 'Polygon', coordinates: [lakeCoords] }
    }
  ];
